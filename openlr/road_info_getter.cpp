...
FormOfAWay RoadInfoGetter::GetFormOfAWay(feature::TypesHolder const & types) const
{
  if (m_trunkChecker(types))
    return FormOfAWay::Motorway;

  if (m_primaryChecker(types))
    return FormOfAWay::MultipleCarriageway;

  return FormOfAWay::SingleCarriageway;
}
}  // namespace openlr
